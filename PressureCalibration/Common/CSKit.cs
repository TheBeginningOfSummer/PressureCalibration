...
namespace CSharpKit
{
    namespace Communication
    {
        ...
        public class SerialPortTool
        {
            ...
            private void MySerialPortDataReceived(object sender, SerialDataReceivedEventArgs e)//接收事件，接收到数据时触发
            {
                ...
                Data.Writer.TryWrite([.. bytesCache]);

                //if (receivedByteCount > 0)
                //{
                //    byte[] b = GetByteArray(dataCache, 0, receivedByteCount);
                //    Data.Writer.TryWrite(b);
            ...
            public byte[] SendWithRead(SendData sendData)
            {
                Thread.Sleep(5);
                MySerialPort.Write(sendData.Data, 0, sendData.Data.Length);
                ...
                dt = DateTime.Now;
                while (receiveList.Count < sendData.ReceiveLength)
                {
            ...
            public byte[] SendWithRead(byte[] data, int receiveLength = 2)
            {
                Thread.Sleep(10);
                MySerialPort.Write(data, 0, data.Length);
                DateTime dt = DateTime.Now;
                ...
                ReadPort(MySerialPort, receiveList);
                dt = DateTime.Now;
                while (receiveList.Count < receiveLength)
                {
                    if (MySerialPort.BytesToRead > 0)
                        ReadPort(MySerialPort, receiveList);
                    Thread.Sleep(10);
        ...
        public class SendData
        {
            ...
            public SendData(byte[] data, int receiveLength)
            {
                Data = data;
    ...
    namespace DataManagement
    {
        ...
        public class BytesTool
        {
            ...
            {
                byte[] bytesCopy = new byte[bytesValue.Length];
                bytesValue.CopyTo(bytesCopy, 0);
                if (isReverse) Array.Reverse(bytesCopy);
                if (index > 8 * bytesCopy.Length - 1) throw new ArgumentOutOfRangeException("index");
                for (int i = 0; i < bytesCopy.Length; i++)
                {
                    if (index > (i * 8 - 1) && index < (i + 1) * 8)
                        return GetBit(bytesCopy[i], (ushort)(index - i * 8));
                }
                return false;
            }
            ...
            /// 得到一个整数截取指定长度位后的值
            /// </summary>
            /// <param name="value">要截取的数值</param>
            /// <param name="bitLength">截取的位长度，从右向左数</param>
            /// <returns></returns>
            public static int GetValueByBit(int value, int bitLength)
            {
    ...
    namespace FileManagement
    {
        ...
        public class FileManager
        {
            ...
            /// <param name="messageArray">表数据</param>
            public static void AppendCSV(string filePath, string fileName, string[] tableHeader, params string[][] messageArray)
            {
                ...
                if (tableHeader != null)
                {
                    ...
                    if (!fileInfo.Exists || fileInfo.Length == 0)
                        AppendFlieString(filePath, fileName, header, FileMode.Append);
                }
                for (int i = 0; i < messageArray.Length; i++)
                {
                    for (int j = 0; j < messageArray[i].Length; j++)
                        message += $"{messageArray[i][j]},";
                    message = message[..^1];
                    message += Environment.NewLine;
                }
                AppendFlieString(filePath, fileName, message, FileMode.Append);
            }
        ...
        public class MessageRecorder
        {
            ...
            public static void RecordError(string error, string solution, string path = "Log", string fileName = "错误记录.csv")
            {
                string rowstr = error;
                if (rowstr.IndexOf('\n') > 0)
        ...
        public class ParameterManager
        {
            ...
            /// <param name="propertyInfo">属性数值集合（属性名-属性值）</param>
            public static void Set<T>(T instance, Dictionary<string, object> propertyInfo)
            {
                if (instance == null) return;
                PropertyInfo[] properties = instance.GetType().GetProperties();
                foreach (PropertyInfo property in properties)
                {
                    if (propertyInfo.TryGetValue(property.Name, out var propertyValue))
                    {
                        if (propertyValue is string propertyString)
                            property.SetValue(instance, propertyString.ConvertTo(property.PropertyType));
                        else
        ...
        public class NotifyRecord
        {
            ...
            public static void Record(string log, LogType logType)
            {
                ...
                {
                    case LogType.Error:
                        FileManager.AppendFlieString("Log\\Error", $"{DateTime.Now:yyy-MM-dd}错误记录.log",
                            $"{DateTime.Now:yyy-MM-dd HH:mm:ss}\t{log}{Environment.NewLine}", FileMode.Append);
                        Notify?.Invoke(log);
                        break;
                    case LogType.Warning:
                        FileManager.AppendFlieString("Log\\Warning", $"{DateTime.Now:yyy-MM-dd}报警记录.log",
                            $"{DateTime.Now:yyy-MM-dd HH:mm:ss}\t{log}{Environment.NewLine}", FileMode.Append);
                        Notify?.Invoke(log);
                        break;
                    case LogType.Modification:
                        FileManager.AppendFlieString("Log\\Modification", $"{DateTime.Now:yyy-MM-dd}更改记录.log",
                            $"{DateTime.Now:yyy-MM-dd HH:mm:ss}\t{log}{Environment.NewLine}", FileMode.Append);
                        Notify?.Invoke(log);
                        break;
                }
